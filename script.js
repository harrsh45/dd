...
function calculateHealthScore() {
    ...
    const upgrades = parseFloat(document.getElementById('upgrades').value);
    const community = parseFloat(document.getElementById('community').value);

    // Validate inputs
    if ([usage, feedback, engagement, support, upgrades, community].some(isNaN)) {
        alert('Please enter valid numbers for all fields');
        return;
    }

    // Calculate the health score using weights
    const healthScore = (
        (usage * 0.3) +
        (feedback * 0.2) +
        (engagement * 0.15) +
    ...
    // Display the score
    document.getElementById('score').innerText = healthScore.toFixed(2);
}
